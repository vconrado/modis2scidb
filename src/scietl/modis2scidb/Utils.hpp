...
// ! Parse band data in the following format:
// <band-num>,<band-alias>[(;<band-num>,<band-alias>)*].
...
// ! Parse a given MODIS file name, decomposing it in base parts as a
// modis_file_descriptor.
modis_file_descriptor  parse_modis_file_name(const std::string& file_name);

// ! Parse a MODIS acquisition date in the following format: AYYYYDDD (YYYY =>
// date year; DDD => julian day)
...
// ! Create a name with a pattern for the subdataset that can be used in format
// expressions to open GDAL subdatasets.
