...
struct modis_file_descriptor {
  std::string data_type_name;     // !< Earth Science Data Type Name (product
                                  // short name).
  std::string acquisition_date;   // !< Date year and julian date (julian date
                                  // of acquisition).
  uint32_t    year;               // !< Date year of acquisition.
  uint32_t    doy;                // !< Julian date of acquisition.
  std::string tile;               // !< Tile identifier.
  uint32_t    tileH;              // !< Tile Horizontal identifier.
  uint32_t    tileV;              // !< Tile Vertical identifier.
  std::string collection_version; // !< Collection version.
  std::string production_date;    // !< Production Date & Time (julian date of
                                  // production).
};
